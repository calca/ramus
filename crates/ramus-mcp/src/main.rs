//! Server MCP di sola lettura sul vault Ramus — vedi
//! specs/M5/2026-09-02-mcp-server-lettura.TODO.md. Binario indipendente
...
impl Server {
    fn new(vault_root: PathBuf, index: Index, search_index: SearchIndex) -> Self {
        Self {
            vault_root,
...
#[tool_handler(
    ...
    name = "ramus-mcp",
    instructions = "Strumenti di sola lettura sul vault di Ramus (journal e pagine markdown): list_journals, read_page, list_pages, search, find_backlinks, list_tags."
)]
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = Server::new(vault_root, index, search_index);
...
mod tests {
    ...
    fn test_server(dir: &TempDir) -> Server {
        ...
        Server::new(vault_root, index, search_index)
    }
